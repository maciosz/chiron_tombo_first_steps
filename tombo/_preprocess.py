...
def _prep_fast5_for_fastq(fast5_data, bc_grp_name, bc_subgrp_name, overwrite):
    ...
    # ~maciosz: pliki testowe chirona nie maja slotu read_id, maja read_number
    # a nasze pliki maja read_id ale niespojne z fastq, wygodniej brac zawsze number
    if True: # if not read_id:
        read_number = th.get_raw_read_slot(fast5_data).attrs.get('read_number')
        read_id = "Read_" + str(read_number)
    try:
...
def _annotate_with_fastqs_worker(
    ...
    while True:
        ...
        # ~maciosz: why up to the underscore?? i need this line to look like this:
        read_id = fastq_rec[0].split()[0].strip()[1:]
